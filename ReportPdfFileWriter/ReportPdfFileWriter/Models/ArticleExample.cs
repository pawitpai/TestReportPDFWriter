...
namespace TestPdfFileWriter
{
    public class ArticleExample
    {
                ...
                )
        {
            ...
            DrawPicturePai(Document, Contents);
            //DrawHeart(Contents);
            //DrawStar(Contents);
            //DrawTextBox(Contents);
            DrawFormPai(Contents);
                ...
                )
        {
            ...
            PdfImage Image1 = new PdfImage(Document, HttpRuntime.AppDomainAppPath + "\\logo.jpg");
                    ...
                    )
        {
            ...
            Contents.Translate(1, 4);




            //////Draw title
            Contents.DrawText(ArialBold, FontSize + 10, 6.25 / 2, 6.7, TextJustify.Center, "WEEKLY REPORT");
            ...
            double margin_y = 0;
            double axis_y_header_column = 6.25 - margin_y - ((FontSize / 100) * 2);


            double vertical_height = (weeklyreport.Months.Count * 0.2) + 0.2;


            double total_height_rectangle = 6.25;

            double height_rectangle = 6.25 - axis_y_header_column;
            double axis_y_rectangle = 6.25 - height_rectangle;

            // draw outline rectangle
            Contents.DrawRectangle(0.0, 6.25 - vertical_height, 6.25, vertical_height, PaintOp.CloseStroke);

            Contents.SetLineWidth(0.01);
            // draw one horizontal line for using in header column
            Contents.DrawLine(0, axis_y_header_column, 6.25, axis_y_header_column);
            ...
            Contents.DrawLine(3, 6.25 - vertical_height, 3, 6.25);
            Contents.DrawLine(4.625, 6.25 - vertical_height, 4.625, 6.25);


            /////Margin for text align right
            double text_align_right = 0.1;

            int multiple = 1;
            double line_plus = 0;
            foreach (MonthData item in weeklyreport.Months)
            {
                Contents.DrawText(ArialBold, FontSize, (double)(0 + 3) / 2, ((axis_y_header_column + 6.25) / 2) - ((FontSize / 100) / 2) - (0.2 * multiple), TextJustify.Center, item.Month);
                Contents.DrawText(ArialNormal, FontSize, 4.625 - text_align_right, ((axis_y_header_column + 6.25) / 2) - ((FontSize / 100) / 2) - (0.2 * multiple), TextJustify.Right, "$" + item.Revenue.ToString("#,###,###"));
                Contents.DrawText(ArialNormal, FontSize, 6.25 - text_align_right, ((axis_y_header_column + 6.25) / 2) - ((FontSize / 100) / 2) - (0.2 * multiple), TextJustify.Right, "$" + item.Forecast.ToString("#,###,###"));

                double y_axis_line_under_text = ((axis_y_header_column + 6.25) / 2) - ((FontSize / 100) / 2) - (0.2 * multiple);

                if (multiple != weeklyreport.Months.Count)
                {
                    Contents.DrawLine(0, y_axis_line_under_text - 0.05, 6.25, y_axis_line_under_text - 0.05);
                }
                line_plus = 0.2;
                multiple++;
            }
            ...
            ////For Y Axis Calculation (text center)
            /////((6 + 6.25) / 2) = horizontal line between text and divide by 2 for get center position between 2 line 
            // but need to minus result of formula - (FontSize / 100) / 2 (by calculate from font size )


            ////For X Axis Calculation (text center)
            /////(0+3)/2 = vertical line between text and divide by 2 for get center position between 2 line 
            Contents.DrawText(ArialBold, FontSize, (double)(0 + 3) / 2, ((axis_y_header_column + 6.25) / 2) - ((FontSize / 100) / 2), TextJustify.Center, "MONTH");
            ...
            ////For Y Axis Calculation (text right)
            /////((6 + 6.25) / 2) = horizontal line between text and divide by 2 for get center position between 2 line 
            // but need to minus result of formula - (FontSize / 100) / 2 (by calculate from font size )


            ////For X Axis Calculation (text right)
            /////(0+3)/2 = vertical line between text and divide by 2 for get center position between 2 line 

            Contents.DrawText(ArialBold, FontSize, 4.625 - text_align_right, ((axis_y_header_column + 6.25) / 2) - ((FontSize / 100) / 2), TextJustify.Right, "REVENUE");
            Contents.DrawText(ArialBold, FontSize, 6.25 - text_align_right, ((axis_y_header_column + 6.25) / 2) - ((FontSize / 100) / 2), TextJustify.Right, "FORECAST");
            ...
            //////Draw Footer
            Contents.DrawText(ArialNormal, FontSize, 6.25, 4.3, TextJustify.Right, weeklyreport.LabelFooter);



            Contents.DrawText(ArialNormal, FontSize, 6.25, 4, TextJustify.Right, weeklyreport.GeneratedOn.ToString("MMM dd,yyyy"));
